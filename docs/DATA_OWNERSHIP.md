...
| Pro-forma Invoice (`ProformaInvoice.jsx`) | ✅ Real, versioned (built 2026-07-21) | `proforma_invoices` table. Also fixed a real bug: `invoiceNo` defaulted from a `localStorage` counter that never synced across staff/devices — the exact "duplicate invoice numbers across staff" risk the settings migration existed to prevent, which had never actually been wired into this file. Now computed via `nextInvoiceNo()` (already existed in `utils.js`, never used) against every invoice number ever saved, queried globally via `loadExistingInvoiceNumbers` (not scoped to one `query_id`, since invoice numbers must be unique business-wide). Save is guarded against the race condition where a user clicks Save before that async computation resolves. **⚠ Superseded (2026-08-22):** merged into `InvoiceGenerator.jsx` (see below) — `ProformaInvoice.jsx` no longer exists. |
| Tax Invoice (`TaxInvoice.jsx`) | ✅ Real, versioned (built 2026-07-21) | `tax_invoices` table. Same invoice-numbering fix as Pro-forma — was defaulting to `` `TAX-2026-${Math.random()...}` ``, a random 3-digit suffix with **zero uniqueness guarantee**, on a document with GST/legal sequential-numbering implications. Also fixed a dead "💾 Save Tax Invoice" footer button (no `onClick` at all). **⚠ Superseded (2026-08-22):** merged into `InvoiceGenerator.jsx` (see below) — `TaxInvoice.jsx` no longer exists. |
| Invoices (`InvoiceGenerator.jsx`) | ✅ Real, versioned (2026-08-22, Phase A+B) | Pro-forma Invoice and Tax Invoice merged into one component with two sub-tabs, same shape as Itinerary's Brief/Detailed — direct instruction, since both need the same Bill To and Tour Details data and having them re-entered twice was the actual complaint. New `invoices` table replaces the two separate ones (`proforma_invoices`/`tax_invoices` dropped, both were empty in production): `doc_type` ('proforma'\|'tax') is the flavor-distinguishing column, mirroring how Itinerary uses `active_tab` — `query_id + doc_type` is each flavor's own stable version-history key, so the two keep fully independent version numbers and numbering prefixes (PI-/TI-) despite sharing one table. Bill To sources from Vendor/Agent Master's `address` field via a picker, with a "Custom" free-text fallback for parties not yet in master data — same pattern as Exchange Order's vendor picker. **Phase B (advance/adjusted payment auto-fetch):** every incoming payment entry in the Payments feature (`EnhancedPaymentTracker.jsx`) gets a real `receipt` number stamped on it the moment it's logged, so `!!entry.receipt` is the literal encoding of "only incoming payments which have a receipt get fetched" (direct instruction) — happens to be true for every entry today since receipt issuance isn't currently conditional, but this is still the correct field to check rather than assuming that always holds. Fetch sums receipted entries matching the invoice's own currency; receipted entries in a *different* currency are surfaced as a count/warning rather than silently included or dropped, since converting them would be a guess. Manual entry stays available underneath and is never silently overwritten by an automatic re-fetch (`advanceIsManual` flag) — the lump-sum-advance-on-series-bookings case, per direct instruction. |
...
- **Phase 3 — Cost Sheet → Quotation, upgraded. ✅ Complete (2026-07-23).** The manual "↻ Pull from Cost Sheet" button (built 2026-07-20) now auto-fires once, only for a genuinely new Quotation (zero saved versions) linked to a Cost Sheet -- safe by construction. A new `pulled_from_cost_sheet_version` column tracks which Cost Sheet version a Quotation was last pulled from. On open, checks for a *newer star-marked* Cost Sheet version beyond that; if one exists, shows a banner + explicit "↻ Pull latest" button -- same mutual, non-silent staleness mechanism as Tour Info's, generalized so the same `pullFromCostSheet` function serves both the original pull and a later targeted re-pull.
- **Phase 4 — Replicate to Brief/Detailed Itinerary, Meal Plan. ✅ Complete (2026-07-23).** Built the shared extraction library first (`parseMealPlanFlags`, `extractItineraryFromCostSheetDays`, `extractHotelsFromCostSheetDays`, `extractItineraryBuilderDaysFromCostSheet` -- all in `utils.js`), then refactored Quotation's own pull to use it too, removing the duplicated inline logic it had before. Both Meal Plan and Itinerary Builder now auto-pull once on creation, same as Quotation -- but from the *star-marked Cost Sheet directly*, not a specific linked `costSheetId`, since neither is launched from a specific Cost Sheet save the way Quotation is (both are opened independently from the toolbar). Same mutual staleness banner + explicit "↻ Pull latest" mechanism, tracked via a new `pulled_from_cost_sheet_version` column on both `meal_plans` and `itineraries`. Itinerary Builder's Brief/Detailed styles share one underlying day-list, so the pull only needs to happen once, not per-style.
- **Phase 5 — Exchange Orders + Tour Briefing Sheet. ✅ Complete (2026-07-23).** Four new extraction functions in `utils.js`: `extractTourBriefingHotelsFromCostSheetDays` (consolidates by hotel with real check-in/check-out dates, made possible by Phase 2's date auto-fill), `extractTourBriefingProgrammeFromCostSheetDays` (day/date/movement + meal flags, narrative "programme" text left blank), `extractTourBriefingTransportSummary` (a free-text summary line, not a table -- matches Tour Briefing Sheet's own field shape), and `extractExchangeOrderDraftsFromCostSheet` (partial order drafts -- route/vehicle/serviceType only). Both documents auto-pull once on creation from the star-marked Cost Sheet and carry the same mutual staleness banner as every other document in the chain, `pulledFromCostSheetVersion` stored inside each document's existing `content` JSONB column rather than a new schema column. Deliberately does **not** attempt to auto-fill guides, flights, trains, or vendor contacts (name/phone/facilitatorId) -- Cost Sheet has no source data for any of that at all (it's vendor-master and staff-assignment data, handled elsewhere), and fabricating placeholders for fields with nothing real behind them would have violated the "leave blank rather than fabricate" principle every pull in this chain follows.
  **⚠ Superseded for Exchange Order (2026-08-20):** Exchange Order was rebuilt from a per-tour-file bundle into an independently versioned per-order document (own `order_no`, own version history, own `vendor_id`) as part of a direct feature spec, and its Cost Sheet auto-pull -- including `extractExchangeOrderDraftsFromCostSheet` -- was deliberately removed at the same time, on Shreshth's explicit direction: an Exchange Order is a service voucher for a vendor, not derived tour-costing data, so pulling draft orders from Cost Sheet's transport/handler rows never fit the actual workflow. Tour Briefing Sheet's auto-pull above is unaffected and still current.
