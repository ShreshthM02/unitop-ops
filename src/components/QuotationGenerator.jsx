...
export default function QuotationGenerator({ query, template, costSheetId, onClose, onSaved, currentUser, readOnly, onUpdateQuery, signatures, docSettings }) {
  ...
  const [q, setQ] = useState({
    ...
    date:        today,
    refLine:     `${query.groupName || query.clientName || ""} — ${query.destination || ""} for ${query.nights ? daysFromNights(query.nights) : "??"}  Days / ${query.nights || "??"} Nights`,
      ...
      <div style={{ background:G.white, width:"min(680px, 100vw)", height:"100vh", overflowY:"auto",
        ...
        {activeTab==='content' && <fieldset ref={fieldsetRef} disabled={readOnly} style={{ flex:1, overflowY:"auto", padding:"16px 20px", border:"none", margin:0, minWidth:0 }}>
          ...
          <div style={{ marginBottom:8 }}>
            ...
            <RichTextEditor value={q.refLine} onChange={v=>setF("refLine",v)} minHeight={40}/>
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:8 }}>
                ...
                <tr key={i}>
                  ...
                  {q.showItinDate && (
                    <td style={{ padding:"3px 4px", width:130 }}>
                      <input type="date" style={{...inputStyle,padding:"3px 5px"}} value={row.date}
                        onChange={e=>updateItinerary(i,"date",e.target.value)}/></td>
                  )}
          ...
          <div>
            ...
            <RichTextEditor value={q.closingSignoff} onChange={v=>setF("closingSignoff",v)} signatures={signatures}/>
          </div>
