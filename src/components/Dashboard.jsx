...
export default function Dashboard({ queries, tours, onOpenQuery, currentUser, onStatClick }) {
  ...
  const onGround   = tours.filter(t=>t.status==="On Ground");
        ...
        <div style={{ background: G.white, borderRadius: 10, border: `1px solid ${G.gray200}`, overflow: "hidden" }}>
          ...
          {queries.filter(q=>!q.cancelled).slice(0, 5).map(q => (
            <div key={q.id} onClick={() => onOpenQuery(q)}
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 13, fontWeight: 500 }}>{q.clientName||q.groupName}</div>
          ...
          {tours.map(t => (
            <div key={t.id} style={{ padding: "10px 16px", borderBottom: `1px solid ${G.gray100}`,
              ...
              <span style={{ fontSize: 10, fontWeight: 600, padding: "2px 8px", borderRadius: 10,
                background: t.status === "On Ground" ? "#EAFAF1" : t.status === "Completed" ? G.gray100 : "#DBEAFE",
                color: t.status === "On Ground" ? "#0E6655" : t.status === "Completed" ? G.gray400 : "#1E40AF" }}>
                {t.status}
              </span>
            </div>
          ))}
          {tours.length===0&&<div style={{padding:24,textAlign:"center",color:G.gray400,fontSize:12}}>No active tours</div>}
        </div>
      </div>
    </div>
  );
