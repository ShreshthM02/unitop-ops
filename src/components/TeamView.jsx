...
export default function TeamView({ queries, staff }) {
  // Real, direct request: team members grouped by role -- Operations
  // first, then Sales, then Accounts, then Admin -- with the highest
  // workload (queries currently assigned to them) shown first within
  // each group, rather than a flat alphabetical list.
  const ROLE_ORDER = ["ops", "sales", "accounts", "admin"];
  const workloadOf = (user) => (queries||[]).filter(q => q.assignedTo === user.id).length;
  const sortedStaff = [...(staff||USERS)].sort((a, b) => {
