...
export default function AgentMaster({ agents, setAgents, queries, payments, currentUser, onSaveAgent, onClose, initialSelectedId }) {
                ...
                <div style={{flex:1,overflowY:"auto",padding:16}}>
                  ...
                  {tab==="history"&&(()=>{
                    ...
                    return <div>
                      ...
                      {filtered.length===0?<div style={{textAlign:"center",padding:32,color:G.gray400,border:`1px dashed ${G.gray200}`,borderRadius:8}}>No queries in this period.</div>:filtered.map(q=>{const pt=payments[q.id];const tv=(parseFloat(pt?.tourValue)||0)*(parseFloat(pt?.roeUsed)||1);const rc=(pt?.entries||[]).reduce((s,e)=>s+(parseFloat(e.amount)||0),0);return<div key={q.id} onClick={()=>document.dispatchEvent(new CustomEvent("unitop-activate-query",{detail:{query:q}}))} style={{background:G.white,border:`1px solid ${G.gray200}`,borderRadius:8,padding:"10px 14px",marginBottom:8,cursor:"pointer"}}><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:tv>0?6:0}}><div style={{flex:1}}><div style={{fontSize:12,fontWeight:600,color:"#1A5276",textDecoration:"underline"}}>{q.groupName||q.clientName}</div><div style={{fontSize:11,color:G.gray400}}>{q.id}{q.tourFileId?" · 📁"+q.tourFileId:""} · {q.destination||q.sector||"—"} · {q.travelDate||"—"}</div></div><StatusBadge status={q.status}/></div>{tv>0&&<div style={{display:"flex",gap:16,fontSize:11}}><span style={{color:G.navy}}>₹ {Math.round(tv).toLocaleString()}</span><span style={{color:"#059669"}}>Rcvd: ₹ {Math.round(rc).toLocaleString()}</span><span style={{color:G.accent,fontWeight:600}}>Bal: ₹ {Math.round(tv-rc).toLocaleString()}</span></div>}</div>;})}
                    </div>;
                  })()}
                  {tab==="ledger"&&(()=>{const ledgerAll=agentLedger(selected);const ledger=ledgerAll.filter(r=>isWithinPeriod(r.travelDate,periodFilter));const tv=ledger.reduce((s,r)=>s+r.tourVal,0);const rc=ledger.reduce((s,r)=>s+r.received,0);const bal=ledger.reduce((s,r)=>s+r.balance,0);return<div><div style={{marginBottom:10}}><TimePeriodFilter value={periodFilter} onChange={setPeriodFilter}/></div><div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:14}}>{[["Total Tour Value","₹ "+Math.round(tv).toLocaleString(),G.navy],["Total Received","₹ "+Math.round(rc).toLocaleString(),"#059669"],["Balance Due","₹ "+Math.round(bal).toLocaleString(),bal>0?G.accent:"#059669"]].map(([l,v,c])=><div key={l} style={{background:G.white,border:`1px solid ${G.gray200}`,borderRadius:8,padding:12}}><div style={{fontSize:10,color:G.gray600,fontWeight:600,textTransform:"uppercase",letterSpacing:"0.5px",marginBottom:4}}>{l}</div><div style={{fontSize:16,fontWeight:700,color:c}}>{v}</div></div>)}</div>{ledger.length===0?<div style={{textAlign:"center",padding:32,color:G.gray400,border:`1px dashed ${G.gray200}`,borderRadius:8}}>No financial data in this period.</div>:ledger.map((r,i)=>{const rq=queries.find(qq=>qq.id===r.queryId);return<div key={r.queryId} style={{background:G.white,border:`1px solid ${G.gray200}`,borderRadius:8,padding:"10px 14px",marginBottom:8}}><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}><div style={{flex:1}}><div onClick={()=>rq&&document.dispatchEvent(new CustomEvent("unitop-activate-query",{detail:{query:rq}}))} style={{fontSize:12,fontWeight:700,color:rq?"#1A5276":G.navy,cursor:rq?"pointer":"default",textDecoration:rq?"underline":"none"}}>{r.tourFileId?"📁 "+r.tourFileId:r.queryId}</div><div style={{fontSize:11,color:G.gray600}}>{r.group} · {r.sector} · {formatDateSlash(r.travelDate)}</div></div><StatusBadge status={r.status}/></div><div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:6,marginBottom:r.entries.length?8:0}}>{[["Tour Value","₹ "+Math.round(r.tourVal).toLocaleString(),G.navy],["Received","₹ "+Math.round(r.received).toLocaleString(),"#059669"],["Balance","₹ "+Math.round(r.balance).toLocaleString(),r.balance>0?G.accent:"#059669"]].map(([l,v,c])=><div key={l} style={{textAlign:"center",background:G.gray50,borderRadius:6,padding:"6px"}}><div style={{fontSize:9,color:G.gray400,fontWeight:600,textTransform:"uppercase",letterSpacing:"0.5px",marginBottom:2}}>{l}</div><div style={{fontSize:12,fontWeight:700,color:c}}>{v}</div></div>)}</div>{r.entries.length>0&&<div style={{borderTop:`1px solid ${G.gray100}`,paddingTop:8}}>{r.entries.map((e,ei)=><div key={ei} style={{display:"flex",gap:10,fontSize:11,color:G.gray600,padding:"3px 0",borderBottom:`1px solid ${G.gray100}`}}><span style={{fontWeight:600,color:G.navy,flexShrink:0}}>{e.receipt||"—"}</span><span style={{fontWeight:600,color:"#059669"}}>₹ {parseFloat(e.amount||0).toLocaleString()}</span><span style={{color:G.gray400}}>{formatDateSlash(e.date)||"—"} · {e.mode||"—"}</span></div>)}</div>}</div>;})}</div>;})()}
                </div>
              </>
