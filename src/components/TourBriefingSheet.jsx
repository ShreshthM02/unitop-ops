...
export default function TourBriefingSheet({ query, template, facilitators, vendors, onClose, currentUser, readOnly, signatures, docSettings }) {
    ...
    loadTourBriefingVersions(db, query.id).then(loaded => {
      if (loaded.length === 0) {
        loadFinalCostSheetVersion(db, query.id).then(finalV => {
          ...
          // Referenced-tour-file fallback, same pattern as Cost Sheet /
          // Quotation / Itinerary's own reference pre-fill. No pricing
          // fields here either (hotels/programme/transport/meals are
          // all structural), so no correction needed after the pull.
  ...
  const buildPrintHTML = (asBlocks) => {
    ...
    const docArgs = {
      title: buildDownloadFilename("Tour Briefing Sheet", "tourbriefing", docSettings, { id: query.id, tourfile: query.tourFileId, group: query.groupName||query.clientName, sector: query.destination || query.sector }),
