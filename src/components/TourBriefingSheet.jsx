...
export default function TourBriefingSheet({ query, template, facilitators, vendors, onClose, currentUser, readOnly }) {
  ...
  const sectionBlocks=(id)=>{
    ...
    const heading=(text)=>`<div style="font-weight:bold;text-decoration:underline;margin:12pt 0 6pt">${text}:</div>`;
    switch(id){
      ...
      case "others": {
        ...
        activeGroups.forEach(g => {
          ...
          blocks.push({ type:"table", headerHTML:`<tr><th>Day</th><th>Date</th><th>Description</th></tr>`,
            rowsHTML: g.items.filter(it=>it.description).map(it=>`<tr><td>${it.day||""}</td><td>${it.date||""}</td><td>${it.description}</td></tr>`) });
        });
      ...
      case "mealplan": {
        ...
        return mealDays.some(d=>d.breakfast||d.lunch||d.dinner||d.itinerary) ? [
          ...
          { type:"table",
            ...
            rowsHTML: mealDays.map(d=>`<tr><td><b>${d.date||""}</b>${d.day?"<br/>("+d.day+")":""}</td><td>${d.itinerary||"—"}</td><td>${d.breakfast||"—"}</td><td>${d.lunch||"—"}</td><td>${d.dinner||"—"}</td>${hasNotes?`<td>${d.notes||""}</td>`:""}</tr>`) },
