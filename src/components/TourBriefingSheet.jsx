...
export default function TourBriefingSheet({ query, onClose }) {
  ...
  const [guides,setGuides]=useState([{id:1,role:"",name:"",phone:"",area:""}]);
  const updG=(i,f,v)=>setGuides(p=>p.map((r,xi)=>xi===i?{...r,[f]:v}:r));
  ...
  const handlePrint=()=>{
    ...
    const sectionHTML=(id)=>{
      ...
      switch(id){
        ...
        case "hotels": return hotels.some(h=>h.hotelName)?`<div style="font-weight:bold;text-decoration:underline;margin:12pt 0 6pt">Hotel Status:</div><table width="100%" border="1" cellpadding="5" cellspacing="0" style="border-collapse:collapse;font-size:10pt"><thead><tr style="background:#1A3A52;color:#fff"><th>Check In</th><th>Check Out</th><th>City</th><th>Hotel Name</th><th>Rooms</th><th>Status</th></tr></thead><tbody>${hotels.filter(h=>h.hotelName||h.city).map(h=>`<tr><td>${h.checkIn||""}</td><td>${h.checkOut||""}</td><td>${h.city||""}</td><td>${h.hotelName||""}</td><td>${h.rooms||""}</td><td>${h.bookingStatus||"Requested"}</td></tr>`).join("")}</tbody></table>${hotelNotes?`<div style="font-style:italic;color:#555;margin-top:4pt">${hotelNotes}</div>`:""}`:""
        case "flights": return flights.some(f=>f.sector)?`<div style="background:#FFE135;display:inline-block;padding:0 3px;font-weight:bold;text-decoration:underline;margin:12pt 0 6pt">Flight Sector Details:</div><table width="100%" border="1" cellpadding="5" cellspacing="0" style="border-collapse:collapse;font-size:10pt"><thead><tr style="background:#1A3A52;color:#fff"><th>Date</th><th>Sector</th><th>Flight No.</th><th>Time</th></tr></thead><tbody>${flights.filter(f=>f.sector).map(f=>`<tr><td>${f.date}</td><td>${f.sector}</td><td>${f.flightNo}</td><td>${f.time}</td></tr>`).join("")}</tbody></table>${flightNotes?`<div style="font-style:italic;color:#555">${flightNotes}</div>`:""}`:""
        case "trains": return trains.some(t=>t.sector)?`<div style="background:#FFE135;display:inline-block;padding:0 3px;font-weight:bold;text-decoration:underline;margin:12pt 0 6pt">Train Details:</div><table width="100%" border="1" cellpadding="5" cellspacing="0" style="border-collapse:collapse;font-size:10pt"><thead><tr style="background:#1A3A52;color:#fff"><th>Date</th><th>Sector</th><th>Train No.</th><th>Name</th><th>Time</th><th>Coach</th></tr></thead><tbody>${trains.filter(t=>t.sector).map(t=>`<tr><td>${t.date}</td><td>${t.sector}</td><td>${t.trainNo}</td><td>${t.trainName}</td><td>${t.time}</td><td>${t.coach}</td></tr>`).join("")}</tbody></table>${trainNotes?`<div style="font-style:italic;color:#555">${trainNotes}</div>`:""}`:""
        ...
        case "others": return otherSvcs.some(s=>s.description)?`<div style="font-weight:bold;text-decoration:underline;margin:12pt 0 6pt">Other Services:</div><table width="100%" border="1" cellpadding="5" cellspacing="0" style="border-collapse:collapse;font-size:10pt"><thead><tr style="background:#1A3A52;color:#fff"><th>Date</th><th>Type</th><th>Description</th><th>Status</th></tr></thead><tbody>${otherSvcs.filter(s=>s.description).map(s=>`<tr><td>${s.date||""}</td><td>${s.serviceType}</td><td>${s.description}</td><td>${s.status}</td></tr>`).join("")}</tbody></table>${otherNotes?`<div style="font-style:italic;color:#555">${otherNotes}</div>`:""}`:""
        case "programme": return programme.some(p=>p.itinerary||p.programme)?`<div style="font-weight:bold;text-decoration:underline;margin:12pt 0 6pt">Programme:</div><table width="100%" border="1" cellpadding="5" cellspacing="0" style="border-collapse:collapse;font-size:10pt"><thead><tr style="background:#1A3A52;color:#fff"><th>DATE & DAY</th><th>PROGRAMME</th><th>BREAKFAST</th><th>LUNCH</th><th>DINNER</th></tr></thead><tbody>${programme.map(p=>`<tr><td><b>${p.date||""}</b>${p.day?"<br/>("+p.day+")":""}</td><td><b style="color:#1A3A52">${p.itinerary||""}</b>${p.programme?"<br/><div style='white-space:pre-line'>"+p.programme+"</div>":""}</td><td>${p.breakfast||"X"}</td><td>${p.lunch||"X"}</td><td>${p.dinner||"X"}</td></tr>`).join("")}</tbody></table>${progNotes?`<div style="font-style:italic;color:#555">${progNotes}</div>`:""}`:""
        case "contacts": return contacts.some(c=>c.contactNo||c.city)?`<div style="font-weight:bold;text-decoration:underline;margin:12pt 0 6pt">Contact List:</div><table width="100%" border="1" cellpadding="5" cellspacing="0" style="border-collapse:collapse;font-size:10pt"><thead><tr style="background:#1A3A52;color:#fff"><th>Date</th><th>City</th><th>Type</th><th>Contact No.</th><th>Address</th></tr></thead><tbody>${contacts.filter(c=>c.contactNo||c.city).map(c=>`<tr><td>${c.date||""}</td><td>${c.city||""}</td><td>${c.vendorType==="Other"?c.vendorTypeOther||"Other":c.vendorType}</td><td>${c.contactNo||""}</td><td>${c.address||""}</td></tr>`).join("")}</tbody></table>${contactNotes?`<div style="font-style:italic;color:#555">${contactNotes}</div>`:""}`:""
        default: return "";
      }
    };
    win.document.write(`<!DOCTYPE html><html><head><title>Tour Briefing Sheet</title>
    <style>body{font-family:'Times New Roman',serif;margin:0;padding:20px 30px;color:#000;font-size:11pt}
    .lh-header{text-align:center;margin-bottom:4pt}
    .lh-logo{height:92pt;width:auto;display:block;margin:0 auto 3pt}
    .lh-addr{font-size:9pt;color:#2a2a2a;line-height:1.65;letter-spacing:0.3pt;text-align:center;margin-bottom:0}
    .lh-rule{height:2pt;border:none;background:linear-gradient(to right,#cb0f0f,#061bb0);margin:7pt 0 10pt;border-radius:1pt}
    .lh-rule-footer{height:0.75pt;border:none;background:linear-gradient(to right,#cb0f0f,#061bb0);margin-bottom:7pt}
    .lh-footer{margin-top:auto;padding-top:10pt}
    .lh-footer-inner{display:flex;align-items:center;justify-content:space-between}
    </style></head><body>
    <div class="lh-header">
      <img src="${LOGO_B64}" class="lh-logo" alt="Unitop"/>
      <div class="lh-addr">Registered Office: 506, DDA-2F, District Centre, Janakpuri, New Delhi, India - 110058</div>
      <div class="lh-addr">Corporate Office: 452, JMD Megapolis, Sec-48, Sohna Rd., Gurugram, Haryana, India - 122018</div>
      <div class="lh-addr">Website: www.unitoptours.com &nbsp;|&nbsp; E-Mail: unitoptours@gmail.com &nbsp;|&nbsp; Telephone:&nbsp;+91&#8209;124&#8209;4476571</div>
      <div class="lh-rule"></div>
    </div>
    ${printOrder.map(id=>sectionHTML(id)).join("")}
    ${footer?`<div style="margin-top:20pt;white-space:pre-line;font-size:10pt">${footer}</div>`:""}
    <div class="lh-footer">
      <div style="height:1.5px;background:linear-gradient(to right,#cb0f0f,#061bb0);margin-bottom:8pt;-webkit-print-color-adjust:exact;print-color-adjust:exact;"></div>
      <div class="lh-footer-inner">
        <img src="${BADGE_MOT_B64}" alt="Ministry of Tourism" style="height:44pt;width:auto;margin-left:72pt"/>
    ...
    <div className="overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{background:G.white,width:880,height:"100vh",display:"flex",flexDirection:"column",boxShadow:"-4px 0 24px rgba(0,0,0,0.15)"}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"14px 18px"}}>
          ...
          {activeTab==="guides"&&<div>{guides.map((g,i)=><div key={g.id} style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr auto",gap:8,marginBottom:8,background:G.gray50,padding:10,borderRadius:8,border:`1px solid ${G.gray200}`}}>{[["Role","role"],["Name","name"],["Phone","phone"],["Area / Cities","area"]].map(([l,k])=><div key={k}><div style={{fontSize:9,color:G.gray600,fontWeight:600,textTransform:"uppercase",letterSpacing:"0.5px",marginBottom:2}}>{l}</div><input style={inp} value={g[k]||""} onChange={e=>updG(i,k,e.target.value)} placeholder={k==="role"?"e.g. Chinese Speaking Guide":""}/></div>)}<div style={{display:"flex",alignItems:"flex-end",paddingBottom:2}}><span style={{cursor:"pointer",color:G.gray400,fontSize:14}} onClick={()=>setGuides(p=>p.filter((_,xi)=>xi!==i))}>✕</span></div></div>)}<button className="btn btn-ghost" style={{fontSize:11}} onClick={()=>setGuides(p=>[...p,{id:Date.now(),role:"",name:"",phone:"",area:""}])}>+ Add Tour Facilitator</button><NoteField val={guideNotes} set={setGuideNotes}/></div>}
        ...
        <div style={{padding:"10px 18px",borderTop:`1px solid ${G.gray200}`,display:"flex",gap:10,flexShrink:0,background:G.gray50}}>
          ...
          <div style={{flex:1,fontSize:11,color:G.gray400,alignSelf:"center"}}>Drag section tiles to reorder in print output</div>
