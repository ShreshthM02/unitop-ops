...
export default function UserProfilePanel({currentUser,onClose,onSave}){
  ...
  const handleSave=async()=>{
    ...
    try {
      // Item 5: avatar photo upload removed entirely per direct
      // decision -- repeated RLS failures despite two separate fix
      // attempts, and judged not worth continuing to debug. Profile now
      // only manages name and a colour swatch, matching how every
      // other avatar in this app already displays when no photo exists
      // -- no 3rd argument here, relying on the RPC's own existing
      // update-only-what's-provided handling rather than passing an
      // explicit null that could conflict with it.
