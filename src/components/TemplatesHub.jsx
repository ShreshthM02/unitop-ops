        ...
        {!isTypo&&sm&&(
          <div>
            ...
            {activePane==="templateContent"&&selectedDoc==="quotation"&&<div>
              ...
              {[["Monument Heading","monumentNote",false],["Domestic Flights Heading","flightsHeading",false],["Domestic Trains Heading","trainsHeading",false],["Remarks Heading","remarksHeading",false]].map(([label,key,multi])=><div key={key} style={{marginBottom:10}}><label style={lbl}>{label}</label>{multi?<textarea style={{...inp,minHeight:52,resize:"vertical"}} value={tmpl.quotation[key]||""} onChange={e=>setTF("quotation",key,e.target.value)}/>:<input style={inp} value={tmpl.quotation[key]||""} onChange={e=>setTF("quotation",key,e.target.value)}/>}</div>)}
              <div style={{marginBottom:10}}>
                <label style={lbl}>Greeting + Opening Line</label>
                <RichTextEditor value={tmpl.quotation.greetingOpening||""} onChange={v=>setTF("quotation","greetingOpening",v)}/>
              </div>
              <div style={{marginBottom:10}}>
                <label style={lbl}>Closing Paragraph + Sign-off</label>
                <RichTextEditor value={tmpl.quotation.closingSignoff||""} onChange={v=>setTF("quotation","closingSignoff",v)}/>
              </div>
