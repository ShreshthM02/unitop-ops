...
describe('Schema completeness: exchange_orders (saveExchangeOrderVersion) -- vendor_id added 2026-08-20 Phase D ("Drawn on" is now a real vendor picker, not free text)', () => {
  const EXPECTED_COLUMNS = ['order_no', 'query_id', 'vendor_id', 'version', 'is_final', 'note', 'content', 'created_by'];
  it('every intended column has a corresponding key in the save payload', async () => {
    const { db, calls } = capturingDb();
    await saveExchangeOrderVersion(db, 'EO-2026-001', 'UTQ-1', 'VND-001', { version: 1, order: {} }, null);
